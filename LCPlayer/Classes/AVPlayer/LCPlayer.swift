...
public class LCPlayerItem: AVPlayerItem { }

public class LCPlayer: AVPlayer {
    // MARK: - Data ----------------------------
    /// player view
    ...
    deinit {
        ...
        removeObserver(self, forKeyPath: LCPlayerObserverKey.status.rawValue)
    ...
    public func setupPlayer() {
        ...
        // 监听player status
        addObserver(self,forKeyPath: LCPlayerObserverKey.status.rawValue,options: .new, context: nil)
    }
...
//        debugPrint("缓冲时长，已缓冲：\(totalLoadTime),总时长：\(duration),缓冲百分比：\(scale)")
    ...
    public override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        // player 的 status
        let keyPathValue = LCPlayerObserverKey(rawValue: keyPath ?? "")
        if let obj = object as? LCPlayer, obj == self {
            if keyPathValue == .status {
                ...
                case .readyToPlay:
                    ...
                    break
                case .failed:
                    playbackDelegate?.playbackDidFailed(player: self, error: LCPlayerError.unknown)
                    break
                default:
                    break
                }
            ...
            case .status:
                ...
                if let item = object as? AVPlayerItem,
                   ...
                   let itemStatus = AVPlayerItem.Status(rawValue: value) {
                    switch itemStatus {
                    case .readyToPlay:
                        if let oneItem = currentItem as? LCPlayerItem {
                            playbackDelegate?.playbackItemReadyToPlay(player: self, item: oneItem)
                        }
                        break
                    case .failed:
                        if let error = item.error {
                            playbackDelegate?.playbackDidFailed(player: self, error: error)
                        }else {
                            playbackDelegate?.playbackDidFailed(player: self, error: LCPlayerError.unknown)
                        }
                        break
